...
export const useBatchDomainPriceENS = (
  domainsList: any,
  duration: number,
  chainId: number
) => {
    ...
    return {
      ...
      queryFn: async () => {
        ...
        const parsedResp = queryResponse?.map((r: any, i: number) => {
          ...
          const price = new BigNumber(r?.data[0]?._hex)?.div(1e18).toFixed(5);
          return {
            ...domain,
            pricePerYear: price,
...
export const genericMulticall = async (
  ...
  const resp = await multicontract.callStatic.tryAggregate(true, calldata);
