...
import {
  SIDARB_DEFAULT_RESOLVER,
  SIDARB_CHAIN_ID,
  SIDARB_REDUX_GRP_KEY,
  SIDARB_TLD,
  SIDARB_IS_USE_GIFT_CARD,
  SIDARB_REFERRAL_INFO,
  SIDARB_EXPLORER_URL,
  SIDARB_PLATFORM_URL,
  SIDARB_DOMAIN_URL,
} from "./config";
...
import {
  useBulkRegisterContractSIDARB,
  useBulkRegisterContractSIDBNB,
} from "@/hooks/Sid/useContracts";
...
function CheckoutConfirmationModal() {
  ...
  const bulkRegisterContract = useBulkRegisterContractSIDARB({
    signer,
    chainId: SIDARB_CHAIN_ID,
  });
  console.log(
    `🚀 ~ file: CheckoutConfirmationModal.tsx:76 ~ bulkRegisterContract:`,
    bulkRegisterContract
  );

  // @ts-ignore
  const platformData = TLD_TO_PLATFORM_DETAILS[SIDARB_TLD];
  ...
  const bulkRegister = async () => {
    ...
    if (wallet?.network?.chainId !== SIDARB_CHAIN_ID) {
      toast.error(
        <div>
          <p>Wrong Network</p>
        </div>,
    ...
    try {
      const tx = await bulkRegisterContract.functions.bulkRegister(
        ...
        [
          SIDARB_REFERRAL_INFO.referrerAddress,
          SIDARB_REFERRAL_INFO.referrerNodehash,
          SIDARB_REFERRAL_INFO.referralAmount,
          SIDARB_REFERRAL_INFO.signedAt,
          SIDARB_REFERRAL_INFO.signature,
        ],
        {
          value: ethers.utils.parseEther(subTotal?.toString()),
        }
          ...
          <div>
            ...
            <p>
              ...
              <a
                ...
                href={`${SIDARB_EXPLORER_URL}/tx/${tx?.hash}`}
              >
                View On Explorer
              </a>
            </p>
                  ...
                  registeredDomains?.map((d: DomainInterFace) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Link
                          href={`${SIDARB_DOMAIN_URL}/${generateTokenId(name)}`}
                        ...
                        {bulkRegisterTxn?.isSuccess &&
                          bulkRegisterTxn?.hash && (
                            <div className="grid grid-cols-2 gap-2 ">
                              <a
                                // @ts-ignore
                                href={`${SIDARB_EXPLORER_URL}/tx/${bulkRegisterTxn?.hash}`}
                                target="_blank"
                                referrerPolicy="no-referrer"
                                className="btn  btn-secondary text-white rounded-full w-full"
                              >
