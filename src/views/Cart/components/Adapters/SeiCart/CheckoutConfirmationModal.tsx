...
import { bulkRegisterDotSeiNft, registerDotSeiNft } from "./helpers";
...
function CheckoutConfirmationModal() {
  ...
  // @ts-ignore
  const platformData = TLD_TO_PLATFORM_DETAILS[SEI_TLD];
  ...
  const handleBulkRegister = async () => {
    ...
    try {
      ...
      console.log(
        `🚀 ~ file: CheckoutConfirmationModal.tsx:107 ~ offlineSigner:`,
        offlineSigner
      );
      // Create a CosmWasmClient
      const signingCosmWasmClient = await getSigningCosmWasmClient(
        RPC_URL[SEI_CHAIN_ID],
        offlineSigner,
        {
          gasPrice: GasPrice.fromString(`${0.25 * checkoutList?.length}usei`),
        }
      );

      console.log(
        `🚀 ~ file: CheckoutConfirmationModal.tsx:120 ~ signingCosmWasmClient:`,
        signingCosmWasmClient
      );

      const tx = await bulkRegisterDotSeiNft({
        signerCosmWasm: signingCosmWasmClient,
          ...
          <div>
            ...
            <p>
              ...
              <a
                ...
                href={`${DOTSEI_EXPLORER_URL}/txs/${tx?.transactionHash}`}
              >
                View On Explorer
              </a>
            </p>
                  ...
                  registeredDomains?.map((d: DomainInterFace) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Link
                          href={`${DOTSEI_PLATFORM_URL}/name/${name}`}
                        ...
                        {registerTxn?.isSuccess && registerTxn?.hash && (
                          <div className="grid grid-cols-2 gap-2 ">
                            <a
                              ...
                              target="_blank"
                              referrerPolicy="no-referrer"
                              href={`${DOTSEI_EXPLORER_URL}/txs/${registerTxn?.hash}`}
                              className="btn  btn-secondary text-white rounded-full w-full"
                            >
                              View on Explorer
                            </a>
                            <a
                              href={`${DOTSEI_PLATFORM_URL}/my-account`}
                              target="_blank"
                              referrerPolicy="no-referrer"
                              className="btn  btn-secondary text-white rounded-full w-full"
                            >
