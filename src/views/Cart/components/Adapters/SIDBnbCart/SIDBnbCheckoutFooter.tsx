...
function SIDBnbCheckoutFooter({}: Props) {
    ...
    <div>
      ...
      <div className="flex justify-between text-xl">
        ...
        {isLoading ? (
          ...
          <p>
            {new BigNumber(fullPrice)?.toFixed(5)} {SIDBNB_COIN_DENOM?.symbol}
          </p>
        )}
      </div>
      ...
      {account && wallet && wallet?.network?.chainId !== SIDBNB_CHAIN_ID && (
        <button
          className="btn btn-primary rounded-full text-white w-full my-2 "
          onClick={async () => {
            try {
              await wallet?.switchChain(SIDBNB_CHAIN_ID);
...
export default SIDBnbCheckoutFooter;
