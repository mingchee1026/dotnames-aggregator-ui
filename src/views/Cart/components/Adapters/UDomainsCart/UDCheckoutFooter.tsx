...
import { UD_CHAIN_ID, UD_COIN_DENOM, UD_REDUX_GRP_KEY } from "./config";
import BigNumber from "bignumber.js";

type Props = {};

function UDCheckoutFooter({}: Props) {
  ...
  const checkoutList = useAppSelector(
    // @ts-ignore
    (s) => s.checkout.checkoutGroupWithPrice[UD_REDUX_GRP_KEY]
  );
    ...
    <div>
      ...
      <div className="flex justify-between text-xl">
        ...
        {isLoading ? (
          ...
          <p>
            {new BigNumber(fullPrice)?.toFixed(5)} {UD_COIN_DENOM.symbol}
          </p>
        )}
      </div>
      {account && wallet?.network?.chainId === UD_CHAIN_ID && (
        <div className="w-full gap-2 flex my-5"></div>
      )}
...
export default UDCheckoutFooter;
