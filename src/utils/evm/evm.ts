...
interface EVMWalletEvents extends WalletEvents {
  accountsChanged(address: (typeof Address)[]): void;
}
...
> {
  ...
  private addresses: (typeof Address)[] = [];
  ...
  async connect({ evmChainId }: ConnectParams = {}): Promise<
    (typeof Address)[]
  > {
  ...
  async sendTransaction(
    ...
    // @ts-ignore
  ): Promise<typeof SendTransactionResult<TransactionReceipt>> {
    if (!this.isConnected()) throw new NotConnected();
  ...
  async signMessage(msg: EthereumMessage): Promise<typeof Signature> {
    if (!this.isConnected()) throw new NotConnected();
    await this.enforcePrefferedChain();
  ...
  getWalletState(): typeof WalletState {
  ...
  getFeatures(): (typeof BaseFeatures)[] {
    return Object.values(BaseFeatures);
  }

  supportsChain(chainId: typeof ChainId): boolean {
